...
hm_node   	equ	<(HFMStart + 0)	; pointer to current node of tree
hm_stream 	equ <(HFMStart + 2)	; pointer to bitstream
hm_tree   	equ <(HFMStart + 4)	; pointer to tree base
hm_byte   	equ <(HFMStart + 6)	; current byte of bitstream
hm_temp		equ hm_byte			; temp location
hm_status 	equ <(HFMStart + 7)	; bits 0-2 = bits left in hm_byte, bit 7 = string with suffix
hm_length	equ <(HFMStart + 8)	; bytes left in current string
node1 		equ leaf1
...
huffmunch_load:	public huffmunch_load
	; hm_node = header
	; Y:X = index
...
huffmunch_read:	public huffmunch_read
	ldy #0
